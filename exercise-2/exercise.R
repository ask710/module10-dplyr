...
vehicles.unique <- length(unique(makes$make))
print(vehicles.unique)
...
vehicles.2.wheel <- filter(vehicles, drive == "2-Wheel Drive" & cty > 20)

# Of those vehicles, what is the vehicle ID of the vehicle with the worst hwy mpg?
# Hint: filter for the worst vecle, then select its ID.
worst.vehicle <- filter(vehicles.2.wheel, hwy == min(hwy))
worst.vehicle.id <- select(worst.vehicle, id)
...
MostEfficientVehicle <- function(my.year, my.make){
  vehicle.year.info <- filter(vehicles, year == my.year, make == my.make)
  highest.hwy <- filter(vehicle.year.info, hwy == max(hwy))
  best.car <- select(highest.hwy, model)
  return(best.car)
}

# What was the most efficient honda model of 1995?
MostEfficientVehicle("1995", "Honda")
